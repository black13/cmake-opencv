...
LineSegmentation::read_image() {
    this->color_img = imread(this->image_path, cv::IMREAD_COLOR);
    this->grey_img = imread(this->image_path, cv::IMREAD_GRAYSCALE);
}
...
LineSegmentation::find_contours() {
    ...
    findContours(img_clone, contours, hierarchy, cv::RETR_LIST, cv::CHAIN_APPROX_NONE, Point(0, 0));
